...
if [[ -n "$proj_dir" ]]
then
        ps aux | egrep -w "\<$proj_dir\>" | grep -v "grep" | grep -v "sshd" | grep -v "$CATALINA_BASE/\(start.sh\|stop.sh\|restart.sh\|logs/catalina.out\)" > /dev/null
        status=$?
        if [[ $status -eq 0 ]] || [[ -s $CATALINA_PID  ]]
        then
                echo "kill it" 
                ps aux | egrep -w "\<$proj_dir\>" | grep -v "grep" | grep -v "sshd" | grep -v "$CATALINA_BASE/\(start.sh\|stop.sh\|restart.sh\|logs/catalina.out\)" |  awk '{print $2}' | xargs -i kill -9 {}
                > $CATALINA_PID
        fi
else
        echo "stop failt!!!"
